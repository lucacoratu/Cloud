...
std::string EncryptionAPI::Decrypt(const string& data, string key)
{
	...
	unsigned char iv[] = { 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff };
	unsigned char* decrypted_data = aes.DecryptCBC((unsigned char*)data.c_str(), static_cast<unsigned int>(data.size()), (unsigned char*)key.c_str(), iv);

	std::string decrypted = "";
	decrypted.append((const char*)decrypted_data, data.size());
