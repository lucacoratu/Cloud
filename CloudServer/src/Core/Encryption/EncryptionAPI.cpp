...
std::string EncryptionAPI::Encrypt(string data, string key)
{
	...
	unsigned char* encrypted_data = aes.EncryptCBC((unsigned char*)data.c_str(), data.size(), (unsigned char*)key.c_str(), iv, out_len);
...
std::string EncryptionAPI::Decrypt(const string& data, string key)
{
	...
	unsigned char iv[] = { 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff };
	unsigned char* decrypted_data = aes.DecryptCBC((unsigned char*)copy_data.c_str(), copy_data.size(), (unsigned char*)key.c_str(), iv);
